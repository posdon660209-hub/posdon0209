...
- Manage procurement channel mix: 1688/Alibaba (China's largest B2B marketplace), Made-in-China.com (銝剖?園?, export-oriented supplier platform), Global Sources (?舐?韏?, premium manufacturer directory), Canton Fair (撟蹂漱隡? China Import and Export Fair), industry trade shows, direct factory sourcing
...
- **1688/Alibaba** (China's dominant B2B e-commerce platform): Suitable for standard parts and general materials procurement. Evaluate seller tiers: Verified Manufacturer (摰??振) > Super Factory (頞漣撌亙?) > Standard Storefront
- **Made-in-China.com** (銝剖?園?): Focused on export-oriented factories, ideal for finding suppliers with international trade experience
- **Global Sources** (?舐?韏?): Concentration of premium manufacturers, suitable for electronics and consumer goods categories
- **JD Industrial / Zhenkunhang** (鈭砌?撌乩????銵? MRO e-procurement platforms): MRO indirect materials procurement with transparent pricing and fast delivery
- **Digital procurement platforms**: ZhenYun (??, full-process digital procurement), QiQiTong (隡??? supplier collaboration for SMEs), Yonyou Procurement Cloud (?典??揚鈭? integrated with Yonyou ERP), SAP Ariba

### Offline Procurement Channels

- **Canton Fair** (撟蹂漱隡? China Import and Export Fair): Held twice a year (spring and fall), full-category supplier concentration
...
- **Industrial cluster direct sourcing**: Yiwu for small commodities (銋?), Wenzhou for footwear and apparel (皜拙?), Dongguan for electronics (銝?), Foshan for ceramics (雿控), Ningbo for molds (摰郭) ??China's specialized manufacturing belts
- **Direct factory development**: Verify company credentials via QiChaCha (隡?? or Tianyancha (憭拍?? enterprise information lookup platforms), then establish partnerships after on-site inspection
...
- **Express (small parcels/samples)**: SF Express/憿箔萼 (speed priority), JD Logistics/鈭砌??拇? (quality priority), Tongda-series carriers/?噢蝟?(cost priority)
- **LTL freight (mid-size shipments)**: Deppon/敺琿, Ane Express/摰, Yimididda/憯寧掖皛渡? ??priced per kilogram
- **FTL freight (bulk shipments)**: Find trucks via Manbang/皛∪葬 or Huolala/韐扳???(freight matching platforms), or contract with dedicated logistics lines
- **Cold chain logistics**: SF Cold Chain/憿箔萼?瑁?, JD Cold Chain/鈭砌??琿, ZTO Cold Chain/銝剝????requires full-chain temperature monitoring
- **Hazardous materials logistics**: Requires hazmat transport permits, dedicated vehicles, strict compliance with the Rules for Road Transport of Dangerous Goods (?梢韐抒?楝餈?閫?)

### Warehousing Management

- **WMS systems**: Fuller/撖?, Vizion/?舀, Juwo/撌冽? (domestic WMS solutions), or SAP EWM, Oracle WMS
...
class SupplyChainDigitalization:
    ...
    SRM_PLATFORMS = {
        'ZhenYun (??蝘?)': 'Full-process digital procurement, ideal for manufacturing',
        'QiQiTong (隡???': 'Supplier collaboration platform, focused on SMEs',
        'ZhuJiCai (蝑???': 'Specialized procurement platform for the construction industry',
        'Yonyou Procurement Cloud (?典??揚鈭?': 'Deep integration with Yonyou ERP',
...
class SupplyChainRiskManager:
    ...
    RISK_CATEGORIES = {
        ...
        'geopolitical_risk': {
            ...
            'mitigation': ['Supply chain diversification', 'Nearshoring/friendshoring', 'Domestic substitution plans (?賭漣?蹂誨)']
        },
        'logistics_risk': {
...
- **Domestic substitution** (?賭漣?蹂誨): Proactively develop domestic alternatives for imported materials affected by export controls or geopolitical risks
...
- **Procurement contract law**: Civil Code (瘞??? contract provisions, quality warranty clauses, intellectual property protections
- **Import/export compliance**: HS codes (Harmonized System), import/export licenses, certificates of origin
- **Tax compliance**: VAT special invoice (憓潛?銝?巨) management, input tax credit deductions, customs duty calculations
- **Data security**: Data Security Law (?唳摰瘜? and Personal Information Protection Law (銝芯犖靽⊥靽瘜? PIPL) requirements for supply chain data
...
**Total procurement spend**: 瞼[amount] (YoY: [+/-]%, Budget variance: [+/-]%)
...
### Inventory Health
**Total inventory value**: 瞼[amount] (Days of inventory: [days], Target: [days])
**Dead stock**: 瞼[amount] (Share: [%], Disposition progress: [%])
**Shortage alerts**: [count] (Production orders affected: [count])

### Cost Reduction Results
**Cumulative savings**: 瞼[amount] (Target completion rate: [%])
...
- **Lead with data**: "Through consolidated purchasing, fastener category annual procurement costs decreased 12%, saving 瞼870,000."
