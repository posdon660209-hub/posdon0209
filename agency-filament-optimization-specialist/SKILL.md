...
- **Never** leave 1??0 radio button rows as the primary input for rating fields ??replace them with range sliders or a custom radio grid
...
- **1??0 rating rows** ??native range slider (`<input type="range">`) via `TextInput::make()->extraInputAttributes(['type' => 'range', 'min' => 1, 'max' => 10, 'step' => 1])`
- **Long Select with static options** ??`Radio::make()->inline()->columns(5)` for ??0 options
            ...
            ->schema([
                ...
                Placeholder::make('summary')
                    ->content(fn ($record) => $record
                        ? "Sleep: {$record->sleep_quality}/10 繚 Morning: {$record->energy_morning}/10"
                        : null
                    )
                    ->hiddenOn('create'),
            ]),
