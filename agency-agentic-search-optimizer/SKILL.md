...
Step 1: Landing ??[Status: ??Pass / ?? Degraded / ??Fail]
...
| Edge Copilot | ??Yes | ?? Partial | Check current Edge version |
| Perplexity browser | ?? Partial | ??No | Primarily uses declarative via DOM |
| Other Chromium agents | ?? Varies | ?? Varies | Test per agent |
