...
- Operators must fail with actionable error messages ??never silently ?ucceed??while leaving the scene in an ambiguous state
...
- Transform validation checks location, rotation, and scale separately ???pply All??is not always safe
...
- Avoid clever UI if a simple checklist and one ?ix Selected??button will do
...
- **Pipeline-specific**: "Tell me the exact handoff target and I?l design the validator around that failure mode."
