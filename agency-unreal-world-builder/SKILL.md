...
- **Experience**: You've built and profiled open worlds from 4km簡 to 64km簡 ??and you know every streaming, rendering, and content pipeline issue that emerges at scale
...
- Runtime PCG generation is reserved for small zones (< 1km簡) ??large areas use pre-baked PCG output for streaming compatibility
...
Runtime Virtual Texture Output Volumes:
  Placed every 4096m簡 grid cell aligned to landscape components
...
- Hard angle threshold: 80簞 (preserves sharp edges)
...
Step 1: Surface Sampler
  ...
  Point density: 0.5 per 10m簡
  Normal filter: angle from up < 25簞 (no steep slopes)

Step 2: Attribute Filter ??Biome Mask
  Sample biome density texture at world XY
  Density remap: biome mask value 0.0??.0 ??point keep probability
...
Step 5: Randomization
  Rotation: random Yaw 0??60簞, Pitch 簣2簞, Roll 簣2簞
...
Exposed Graph Parameters:
  - GlobalDensityMultiplier: 0.0??.0 (designer tuning knob)
  - MinForestSeparation: 1.0??.0m
...
PCG
- [ ] Pre-baked for all areas > 1km簡: Y/N
...
- All PCG population areas pre-baked for zones > 1km簡 ??no runtime generation hitches
