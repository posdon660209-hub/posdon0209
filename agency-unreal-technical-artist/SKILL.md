...
Emitter: Burst ??15??5 particles on spawn, 0 looping
...
Modules:
  ...
  Initial Velocity:
    Cone direction upward, 45簞 spread
    Speed: Uniform(150, 350) cm/s

  Gravity Force: -980 cm/s簡
...
Input: Landscape Surface Sampler
  ??Density: 0.8 per 10m簡
  ??Normal filter: slope < 25簞 (exclude steep terrain)

Transform Points:
  ??Jitter position: 簣1.5m XY, 0 Z
  ??Random rotation: 0??60簞 Yaw only
...
Parameters exposed to level:
  - GlobalDensityMultiplier (0.0??.0)
  - MinSeparationDistance (1.0??.0m)
